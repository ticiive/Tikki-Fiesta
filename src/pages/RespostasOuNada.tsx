...
const RESPOSTAS_KEY = 'tikki-fiesta-respostas-state';

const RespostasOuNada = () => {
  const location = useLocation();
  const navigate = useNavigate();

  const effectiveState = (location.state as any) ?? (() => {
    try { const s = localStorage.getItem(RESPOSTAS_KEY); return s ? JSON.parse(s) : null; } catch { return null; }
  })();
  ...
  useEffect(() => {
    if (location.state) {
      try { localStorage.setItem(RESPOSTAS_KEY, JSON.stringify(location.state)); } catch {}
    }
  }, []); // eslint-disable-line
  ...
  useEffect(() => {
    if (phase === 'running' && timeLeft <= 5 && timeLeft > 0) beep();
  }, [timeLeft, phase]);
  ...
  const isUrgent = timeLeft <= 5 && phase === 'running';
          ...
          <div className="flex flex-col items-center gap-4 px-4 py-5">
            ...
            <div style={{
              ...
              <p style={{
                ...
                {phase === 'waiting' ? 'Vez de responder:' : phase === 'running' ? 'Respondendo...' : 'Tempo!'}
              </p>
              <p style={{
                fontFamily: 'Fredoka, sans-serif',
                fontWeight: 700,
            ...
            <div className="relative flex items-center justify-center"
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center"
                ...
                <span style={{
                  ...
                  {phase === 'waiting' ? String(DURATION).padStart(2, '0') : label}
                </span>
                {phase === 'done' && (
                  <span style={{
                    fontFamily: 'Fredoka, sans-serif',
...
export default RespostasOuNada;
